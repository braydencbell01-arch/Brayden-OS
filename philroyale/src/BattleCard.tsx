...
export function BattleCard({ character, size = 'hand', dimmed }: Props) {
  ...
  return (
    <div
      className={`card-frame relative overflow-hidden rounded-md ${next ? 'h-14 w-11' : 'aspect-[3/4] w-full'} ${dimmed ? 'opacity-55' : ''}`}
    >
      <div
        className="absolute inset-[3px] flex flex-col overflow-hidden rounded-sm"
        style={{
          background: `linear-gradient(165deg, ${artLit} 0%, ${art} 55%, #1a1410 140%)`,
        }}
      >
        <div className="relative flex flex-1 items-center justify-center">
          ...
          />
          {!next ? (
            <span className="absolute bottom-0.5 left-0 right-0 truncate px-0.5 text-center text-[0.55rem] font-extrabold leading-tight text-white drop-shadow-[0_1px_1px_#000]">
              {character.name}
            </span>
          ) : null}
        </div>
